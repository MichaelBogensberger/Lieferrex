...
public interface GerichtRepository extends JpaRepository<Gericht,Long> {
    ...
    @Query(value = "SELECT * FROM gericht WHERE gericht.mandant_id = (SELECT m.mandant_id FROM mandant as m INNER JOIN angestellter as a ON a.mandant_id = m.mandant_id WHERE a.mandant_id = ?1 AND status = 0)", nativeQuery = true)
